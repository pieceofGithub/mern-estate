...
export const signin =  async(req, res, next) => {
  ...
  try {
    ...
    if(!validUser) return next(errorHandler(404,"kullanıcı bulunamadı"))
    const validPassword = bcryptjs.compareSync(password, validUser.password)
    if(!validPassword) return next(errorHandler(401, "hatalı bilgi email ya da şifre"))
...
export const signOut = async (req, res, next) => {
  try {
    ...
    res.status(200).json('Kullanıcı çıkış yaptı!');
  } catch (error) {
    next(error);
  }
};